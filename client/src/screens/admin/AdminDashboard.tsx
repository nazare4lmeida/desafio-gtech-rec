...
export default function AdminDashboard() {
  ...
  const cards = useMemo(
    () => [
      { val: stats.total, label: 'Total de Submissões', accent: 'border-t-blue' },
      { val: stats.passed, label: 'Aprovados', accent: 'border-t-green' },
      { val: stats.failed, label: 'Reprovados', accent: 'border-t-red' },
      { val: `${stats.avgPct}%`, label: 'Média Geral', accent: 'border-t-gold' },
      { val: stats.modules.recovery, label: 'Recuperação', accent: 'border-t-gold' },
      { val: stats.modules.presenca, label: 'Presença', accent: 'border-t-blue' },
    ],
    [stats],
  )
  ...
  return (
    <div className="animate-fade-up">
      <div className="flex items-start justify-between gap-4 flex-wrap mb-6">
        <div>
          <h2 className="font-display text-[1.3rem] font-extrabold text-navy">Dashboard</h2>
          <p className="text-[.83rem] text-muted">Visão geral consolidada dos módulos e testes.</p>
          <p className="text-[.74rem] text-muted mt-1">
            Painel agora lê os resultados persistidos pela API, incluindo Recuperação e Desafio Presença.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-6 gap-4 mb-6">
        {cards.map((card) => (
          ...
          >
            <div className="font-display text-[2rem] font-extrabold text-navy">{card.val}</div>
      ...
      <div className="grid lg:grid-cols-[1.2fr_.8fr] gap-5 mb-6">
        <div className="bg-surface rounded-[14px] border border-border shadow-card p-5">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            {[
              ['Teste Full-Stack', stats.modules.fullstack, 'text-blue', 'bg-[#EAF3FB]'],
              ['Prova de Recuperação', stats.recovery.total, 'text-gold', 'bg-gold-bg'],
              ['Desafio Presença', stats.presenca.total, 'text-green', 'bg-green-bg'],
              ...
              <div key={String(label)} className={`rounded-xl p-4 ${bgClass}`}>
                ...
                <div className="text-2xl font-display font-extrabold text-navy mt-2">{value}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3 text-[.83rem]">
            <div className="flex items-center justify-between rounded-xl bg-[#EFF4FA] p-3">
              ...
              <strong className="text-navy">
                {stats.recovery.total ? `${Math.round((stats.recovery.passed / stats.recovery.total) * 100)}%` : '0%'}
              </strong>
            </div>
            <div className="flex items-center justify-between rounded-xl bg-[#EFF4FA] p-3">
            ...
            <div className="flex items-center justify-between rounded-xl bg-[#EFF4FA] p-3">
              ...
              <strong className="text-navy">{Object.keys(stats.categories).length}</strong>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-[14px] border border-border shadow-card overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-3">
          <span className="text-[.9rem] font-bold text-navy">Últimas Submissões</span>
          {loading && <span className="text-[.72rem] text-muted">Atualizando…</span>}
        </div>
        ...
        {latest.length ? (
          <table className="w-full border-collapse">
            ...
            <tbody>
              {latest.map((row) => (
                <tr key={`${row.module}-${row.id}`} className="border-t border-border hover:bg-[#F8FBFF]">
                  <td className="px-4 py-3 text-[.82rem]">
                    <strong>{row.name}</strong>
                    <br />
                    <span className="text-muted text-[.75rem]">{row.email}</span>
                  </td>
                  <td className="px-4 py-3 text-[.82rem]">
                  ...
                  <td className="px-4 py-3 text-[.82rem]">
                    ...
                    <span className="text-muted ml-1">({pct(row.score, row.max)}%)</span>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-[.7rem] font-bold ${
                        row.passed ? 'bg-green-bg text-green' : 'bg-red-bg text-red'
                      }`}
                    >
