...
export default function StudyGuideScreen() {
  ...
  const questions = useMemo<StudyQuestion[]>(() => {
    // Tratamento rigoroso das questões da prova
    const proofQuestions = RECOVERY_QUESTIONS.map((q) => ({
      id: `proof-${q.id}`,
      topic:
        q.category.toUpperCase() === "BANCO DE DADOS"
          ? "Banco de Dados"
          : q.category,
      prompt: q.text.replace(/<code>/g, "").replace(/<\/code>/g, ""),
      options: q.options || [],
      correct: q.correct,
      explanation: `Gabarito: ${q.options[q.correct]}. Estude esse conceito para fixar o aprendizado.`,
      ...
      <div className="bg-surface rounded-[24px] border border-border shadow-card p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          <div className="rounded-2xl border border-border bg-[#EFF4FA] p-4">
            ...
            <ol className="mt-3 space-y-2 text-sm text-muted leading-6 list-decimal pl-5">
              <li>Leia o resumo de cada tema e pratique os exemplos.</li>
              <li>Responda as perguntas com atenção ao feedback.</li>
              <li>Pratique a lógica dos desafios propostos.</li>
            </ol>
          </div>
          ...
          <div className="rounded-2xl border border-green-bdr bg-green-bg p-4">
            ...
            <p className="text-sm text-muted mt-2">
              Acertos mínimos para aprovação.
            </p>
          </div>
        ...
        <section className="mt-10">
          ...
          <p className="text-muted text-sm mt-1 mb-4">
            Questões da prova oficial + extras para reforço.
          </p>
            ...
            {questions.map((question, index) => {
              ...
              return (
                ...
                >
                  ...
                  <p
                    className="text-navy font-semibold leading-7 mb-4"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {question.options.map((option, idx) => {
                      ...
                      const showCorrect =
                        isRevealed && idx === question.correct;
                      const showWrong =
                        isRevealed && picked && idx !== question.correct;
                      ...
                      return (
                        <button
                          ...
                          className={`text-left rounded-2xl border px-4 py-3 text-sm transition-all ${
                            showCorrect
                              ? "border-green-bdr bg-green-bg text-green font-bold"
                              : showWrong
                  ...
                  <div className="mt-4 flex items-center gap-3">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-navy text-white text-sm font-semibold hover:bg-blue"
                    >
                  ...
                  {isRevealed && (
                    <div className="mt-4 rounded-2xl border border-border bg-[#EFF4FA] p-4 text-sm text-muted leading-7">
                      <p>
                        <span className="font-bold text-navy">Explicação:</span>{" "}
        ...
        <section className="mt-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[280px,1fr] gap-4 mt-4">
            <div className="rounded-2xl border border-border bg-white p-3 h-fit">
              {CHALLENGE_EXAMPLES.map((challenge) => (
                <button
                  ...
                  className={`w-full text-left rounded-2xl px-4 py-4 mb-2 border transition-all ${
                    challenge.id === openChallengeId
            ...
            <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
              ...
              <p className="text-sm text-muted mt-2">{activeChallenge.goal}</p>
              <div className="mt-5 grid grid-cols-1 xl:grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-bold mb-2 uppercase text-muted">
                    Código inicial
                  </p>
