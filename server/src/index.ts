...
const DEFAULT_QUESTIONS: Question[] = [
  { id:1, order:1, difficulty:'beginner', category:'frontend',
    ...
    options:['<link>','<a>','<href>','<button>'], correct:1,
    feedbackOk:'<strong>Correto!</strong> <code>&lt;a&gt;</code> é a tag de âncora do HTML.',
    feedbackNok:'<strong>Quase!</strong> A tag correta é <code>&lt;a&gt;</code> (âncora).' },
  { id:2, order:2, difficulty:'beginner', category:'geral',
    text:'O que significa a sigla CSS?',
    options:['Creative Style Sheets','Computer Style System','Cascading Style Sheets','Coded Style Sheets'], correct:2,
  ...
  { id:3, order:3, difficulty:'intermediate', category:'lógica',
    ...
    feedbackOk:'<strong>Correto!</strong> Retorna <code>"object"</code> por retrocompatibilidade.',
    feedbackNok:'<strong>Quase!</strong> <code>typeof null</code> retorna <code>"object"</code>.' },
  { id:4, order:4, difficulty:'intermediate', category:'backend',
    ...
    options:['PUT','POST','DELETE','PATCH'], correct:3,
    feedbackOk:'<strong>Correto!</strong> <code>PATCH</code> atualiza parcialmente.',
    feedbackNok:'<strong>Quase!</strong> O método correto é <code>PATCH</code>.' },
  { id:5, order:5, difficulty:'hard', category:'frontend',
    ...
    feedbackOk:'<strong>Correto!</strong> O Virtual DOM melhora a performance do React.',
    feedbackNok:'<strong>Quase!</strong> O Virtual DOM é uma representação em memória.' },
]
...
function deleteRow(module: AdminResultRow['module'], id: number) {
  if (module === 'fullstack') db.results = db.results.filter((r) => r.id !== id)
  if (module === 'recuperacao') db.recoveryResults = db.recoveryResults.filter((r) => r.id !== id)
  if (module === 'presenca') db.presencaResults = db.presencaResults.filter((r) => r.id !== id)
}
...
app.post('/api/recovery-results', (req, res) => {
  ...
  const existing = db.recoveryResults.find((r) => r.email.toLowerCase() === email)
  if (existing) return res.status(409).json({ error: 'Já submetido', existing })
...
app.post('/api/presenca-results', (req, res) => {
  const email = String(req.body.email || '').trim().toLowerCase()
  const existing = db.presencaResults.find((r) => r.email.toLowerCase() === email)
  if (existing) return res.status(409).json({ error: 'Já submetido', existing })

  const previousPct = Number(req.body.previousPct) || 0
  const challengePct = Number(req.body.challengePct) || 0
  const presencaPct = Math.max(previousPct, Number(req.body.presencaPct) || 0, challengePct)
...
app.delete('/api/admin-results', (req, res) => {
  ...
  rows.forEach((row: { id?: number; module?: AdminResultRow['module'] }) => {
    if (typeof row?.id === 'number' && row?.module) deleteRow(row.module, row.id)
  })
  saveDB()
...
app.get('/api/stats', (_req, res) => {
  ...
  const avg = total
    ? Math.round(allResults.reduce((sum, r) => sum + Math.round((r.score / r.max) * 100), 0) / total)
    : 0

  const cats: Record<string, { correct: number; total: number }> = {}
  db.results.forEach((r) => Object.entries(r.cats).forEach(([c, v]) => {
  ...
  res.json({
    ...
    presenca: {
      ...
      avgPct: db.presencaResults.length
        ? Math.round(db.presencaResults.reduce((s, r) => s + r.presencaPct, 0) / db.presencaResults.length)
        : 0,
    },
  })
})
